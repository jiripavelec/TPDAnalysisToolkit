...
class RawDataWrapper():
    ...
    def running_average(self, x, N):
        ...
        temp = np.insert(temp,0,x[:N-1])
        temp = np.append(temp,x[N:-1:-1])
        return temp
    ...
    def getRawData(self, desiredMasses):
        temp = np.array(self.m_parsedData[(self.m_listOfColumns.index('temperature')),:])
        for i in self.massListToIndices(desiredMasses):
            temp = np.vstack((temp, self.m_parsedData[i,:]))
        return temp
